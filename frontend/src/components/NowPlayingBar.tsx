...
export function NowPlayingBar({ queue }: NowPlayingBarProps) {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-40 bg-stone-800/98 backdrop-blur-md border-t border-stone-700/50 shadow-[0_-4px_20px_rgba(0,0,0,0.3)] px-4 py-3 cursor-pointer"
      onClick={() => navigate(`/${slug}/queue`)}
    >
      <div className="flex items-center gap-3 max-w-lg mx-auto">
        <div className="w-11 h-11 rounded-xl overflow-hidden flex-shrink-0 bg-stone-700">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-white text-sm font-medium truncate">{current.song.title}</p>

...
run_migrations() {
  ...
  if [ -n "$DATABASE_URL" ]; then
    ...
    psql "$MIGRATION_URL" -f scripts/add-proxy-refund.sql 2>/dev/null || true
  else
    ...
    PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USERNAME" -d "$DB_NAME" -f scripts/add-proxy-refund.sql 2>/dev/null || true
  fi
